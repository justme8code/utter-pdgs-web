...
import {Box, CalendarDays, Clock, Hash, Package, Tag} from "lucide-react"; // Icons
            ...
            {purchase && purchase.id ? (
                <div className="space-y-4">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                        ...
                        <ReadOnlyInput label="Total Usable"
                                       value={`${purchase.usableWeight} ${purchase.rawMaterial?.uom}`} icon={Box}/>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {conversion.fields.map((field, index) => (
                                    !isNaN(parseFloat(String(field.kgUsed))) && field.kgUsed !== 0.0
                                        ? <ReadonlyConversion key={field.ingredient?.id || index} field={field}
                                                              showLabel={index === 0}/>
                                        : null
                                ))}
                            </div>
                ...
                <p className="text-sm text-muted-foreground italic">Loading purchase details or purchase not
                    found...</p>
            )}
        </div>
    );
